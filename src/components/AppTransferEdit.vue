...
export default defineComponent({
  ...
  methods: {
    async loadCard() {
      ...
      if (this.edit) {
        ...
        if (this.card) {
          ...
          let region = this.card.region;
          let regionTo = this.card.regionTo;
          ...
          console.log(this.regionTo);
          for (let i = 0; i < this.regions.length; i++) {
            let reg = this.regions[i];
            if (reg.name == region) {
              this.region = reg;
            }
            if (reg.name == regionTo) {
              this.regionTo = reg;
            }
          }
        }
    ...
    handleFilesUpload() {
      if (this.clone || this.$route.query.edit) {
        this.img = ``;
        if (this.files) {
